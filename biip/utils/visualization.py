...
def plot_true_pred_cylinder(
    title, grid_size_i, grid_size_j, dataset, f_hat, save_path, fix_scale=False
):
    figure_title = title
    ...
    f_interior = (
        dataset["f_interior"].numpy().reshape(num_timesteps, grid_size_i, grid_size_j)
    )
    f_boundary = (
        dataset["f_boundary"].squeeze().numpy().reshape(num_timesteps, grid_size_i, 2)
    )
    ...
    for i in range(dataset["num_nodes_boundary"] // 2):
        ...
        axs[i, 0].plot(
            [t for t in range(num_timesteps)],
            f_boundary[:, i, 0],
            color="limegreen",
            linestyle="dotted",
            linewidth=4,
